...
const ImageTracer = require('/home/dev/public/javascript/imagetracer_v1.2.1');
...
router.post('/', (req, res) => {
    ...
    execSync(`python2 ./python/01_crop.py --src_dir=${scan_dir} --dst_dir=${crop_dir} --txt=./python/399-uniform.txt`);
    training_progress.push("image cropped");
    
    execSync(`python2 ./python/02_font2image.py --src_font=./python/NanumGothic.ttf --dst_font=./python/NanumGothic.ttf --sample_dir=${sample_dir} --handwriting_dir=${crop_dir}`);
    training_progress.push("image created");
    
    execSync(`python2 ./python/03_package.py --dir=${sample_dir} --save_dir=${data_dir}`)
    training_progress.push("data compressed");
    
    execSync(`python2 ./python/04_train.py --experiment_dir=${root_dir} --experiment_id=0 --batch_size=16 --lr=0.001 --epoch=60 --sample_steps=100 --schedule=20 --L1_penalty=100 --Lconst_penalty=15 --freeze_encoder=1`);
    training_progress.push("first trained");
    
    execSync(`python2 ./python/04_train.py --experiment_dir=${root_dir} --experiment_id=0 --batch_size=16 --lr=0.001 --epoch=120 --sample_steps=100 --schedule=30 --L1_penalty=500 --Lconst_penalty=1000 --freeze_encoder=1`);
    training_progress.push("secondPhaseTrained");

    execSync(`python2 ./python/05_infer.py --model_dir=${model_dir} --batch_size=1 --source_obj=${data_dir}/val.obj --embedding_ids=0 --save_dir=${result_dir} --progress_file=${logs_dir}/progress`);
