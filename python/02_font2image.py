...
# 주어진 폰트를 사용하여 글자를 그리고 이들의 평균 오프셋을 계산.
# 주어진 폰트가 주어진 글자들을 적절히 배치하고 그려내는지 확인하기 위한 목적으로 사용
def get_font_offset(charset, font, canvas_size, filter_hashes):
    _charset = charset[:]
    np.random.shuffle(_charset)
    sample = _charset[:2000]
    font_offset = np.array([0, 0], dtype=np.float64)
    count = 0
    for c in sample:
...
def draw_handwriting(char, src_font, canvas_size, src_offset, dst_folder):
    import codecs
    
    byte_string = char.encode('utf-8')
    encoded_unicode = codecs.decode(byte_string, 'utf-8').encode('raw_unicode_escape')
    s = encoded_unicode.decode().replace("\\u", "").upper()
    ...
    # check the filter example in the hashes or not
    src_img = draw_single_char(char, src_font, canvas_size, src_offset[0], src_offset[1])
    example_img = Image.new("RGB", (canvas_size * 2, canvas_size), (255, 255, 255)).convert('L')
    example_img.paste(dst_img, (0, 0))
    example_img.paste(src_img, (canvas_size, 0))
...
def font2img(src, dst, charset, char_size, canvas_size,
             x_offset, y_offset, sample_count, sample_dir, label=0, filter_by_hash=True,  handwriting_dir=False):
    import codecs
    ...
    if handwriting_dir:
        ...
        train_set = []
        for sample_char in charset:
            print(sample_char)
            e = draw_handwriting(sample_char, src_font, canvas_size, [x_offset, y_offset], handwriting_dir)

            if e:
                byte_string = sample_char.encode('utf-8')
                encoded_unicode = codecs.decode(byte_string, 'utf-8').encode('raw_unicode_escape')
                mdf_c = encoded_unicode.decode().replace("\\u", "").upper()
                
                e.save(os.path.join(sample_dir, "%d_%s_train.png" % (label, mdf_c)))
                train_set.append(sample_char)
                count += 1
                if count % 100 == 0:
                    print("train: processed %d chars" % count)
        ...
        for c in charset:
            ...
            if e:
                ...
                e.save(os.path.join(sample_dir, "%d_%s_val.png" % (label, mdf_c)))
                count += 1
                if count % 100 == 0:
                    print("val : processed %d chars" % count)
        return


parser = argparse.ArgumentParser(description='Convert font to images')
# PATH
parser.add_argument('--data_dir',           dest='data_dir',        type=str,   default=osp.join(os.getcwd(), 'python', 'data'), help='data folder directory')
parser.add_argument('--src_font',           dest='src_font',        type=str,   default='NanumGothic.ttf', help='path of the source font')
parser.add_argument('--dst_font',           dest='dst_font',        type=str,   default='NanumGothic.ttf', help='path of the target font')
parser.add_argument('--sample_dir',         dest='sample_dir',                  default='', help='directory to save examples')
parser.add_argument('--handwriting_dir',    dest='handwriting_dir', type=str,   default='',
                    help='pick handwriting samples (399 training set). Note that this should not be used with --suffle.')
...
if __name__ == "__main__":
    ...
    args.sample_dir = osp.join(args.data_dir, 'pair')
    args.handwriting_dir = osp.join(args.data_dir, 'handwriting')
    args.src_font = osp.join(args.data_dir,  args.src_font)
    args.dst_font = osp.join(args.data_dir,  args.dst_font)
