...
class UNet(object):
    def __init__(self, experiment_dir=None, experiment_id=0, batch_size=16, input_width=128, output_width=128,
        ...
        # init all the directories2
        self.sess = None
        # experiment_dir is needed for training
        if experiment_dir:
            self.data_dir = osp.join(os.getcwd(), 'python', 'data', 'package')
            self.checkpoint_dir = os.path.join(self.experiment_dir, 'checkpoint', f'experiment_{self.experiment_id}_batch_{self.batch_size}')
            self.sample_dir = os.path.join(self.experiment_dir, "sample", f'experiment_{self.experiment_id}_batch_{self.batch_size}')
            self.log_dir = os.path.join(self.experiment_dir, "logs", f'experiment_{self.experiment_id}_batch_{self.batch_size}')
            ...
            try:
                number = re.findall(r'\d+', sorted(os.listdir(self.checkpoint_dir))[1])[1]
                if number == '107850':
                    pass
                
            except IndexError:
                baseline_ckpt_dir = osp.join(os.getcwd(), 'baseline_checkpoint', 'experiment_0_batch_16')
                for fileName in os.listdir(baseline_ckpt_dir):
                    shutil.copy(src=osp.join(baseline_ckpt_dir, fileName),
                                dst=osp.join(self.checkpoint_dir, fileName))
    ...
    def get_model_id_and_dir(self):
        ...
        # model_dir = osp.join(self.experiment_dir, 'baseline', 'checkpoint', model_id)
    ...
    def validate_model(self, val_iter, epoch, step):
        ...
        sample_img_path = os.path.join(self.sample_dir, "sample_%02d_%04d.png" % (epoch, step))
