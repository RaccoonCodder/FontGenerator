...
parser.add_argument('--dir',            dest='dir',         default='', help='path of examples')
...
if __name__ == "__main__":
    args.dir = osp.join(args.data_dir, 'pair')
    args.save_dir = osp.join(args.data_dir, 'package')
    
    if not osp.exists(args.save_dir):
        os.makedirs(args.save_dir)
    
    train_path = osp.join(args.save_dir, "train.obj")
    val_path = osp.join(args.save_dir, "val.obj")
    pickle_examples(glob.glob(osp.join(args.dir, "*.png")), train_path=train_path, val_path=val_path,
